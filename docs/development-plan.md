...
- [x] 家长主页 UI（紫色主题 + 核心数据 + 模块成绩 + 最近训练）
- [ ] 邀请码关联流程（学生展示邀请码 + 家长输入关联）→ Phase 5
- [x] 家长查看孩子打卡日历（通过 `?role=parent` 复用现有 API）
- [x] 家长查看训练详情（复用 history API）
- [x] 家长查看成绩趋势（通过 stats API）
- [x] 家长查看错题本（通过 mistakes API + `?role=parent`）

### 4.3 后台管理 UI

- [x] Admin 布局（深色侧边栏导航）
- [x] 古文篇目管理（新增/编辑/删除/搜索/筛选 + 弹窗表单）
- [x] AI 模型配置页（模块配置卡片 + 编辑弹窗 + Key 脱敏）
- [x] 用户管理页（用户列表 + 角色标签 + 训练/提交统计）
- [x] 管理后台概览页（统计卡片）
- [x] Admin 权限校验（`requireRole` 函数）
...
- [x] 本地环境下，所有功能全部可用（39 路由编译通过）
- [x] 学生、家长、管理员三个角色流程均已验证
- [ ] 折线图正确展示近期成绩趋势（待 Phase 5 添加）
